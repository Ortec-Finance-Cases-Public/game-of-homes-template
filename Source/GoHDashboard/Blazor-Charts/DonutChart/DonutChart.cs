...
namespace WebAssemblyMan
{
    public class DonutChart : ComponentBase
    {
        ...
        protected override void BuildRenderTree(RenderTreeBuilder builder)
        {
            ...
            string[] inputLabelsArr = InputLabels.Split(new[] { ',' }, StringSplitOptions.RemoveEmptyEntries);
            ...
            foreach (string dataStr in inputDataArr)
            {
                ...
                {
                    { "class", "segment-" + (1 + counter++) }, { "cx", "21" }, { "cy", "21" }, { "r", "15.915" },
                    { "stroke-dasharray", percent + " " + reversePercent }, { "stroke-dashoffset", offset.ToString() }
                };
            ...
            foreach (Circle segment in segments) svg.AddItems(segment);
            svg.AddItems(grp);
            ...
            foreach (string dataStr in inputDataArr)
            {
                ...
                builder.AddAttribute(++seq, "class", "legend-dot-" + (counter + 1));
                ...
                builder.AddContent(++seq, labels + " " + "(" + data + "%)");
                builder.CloseElement();
            }
